  ...
  <body>
    ...
    <div class="container">
      ...
      <h5>
        ...
        (<?php
          $get = $pdo->prepare("SELECT * FROM contact");
          $get->execute();
          echo $get->rowCount();
        ?>) - Contacts
      </h5>
      ...
      <?php
        $get = $pdo->prepare("SELECT * FROM contact ORDER BY id DESC");
        $get->execute();
        if($get->rowCount() > 0){
          while($f = $get->fetch()){
      ?>
      ...
      <table id="table" class="table table-striped table-bordered">
        <tr>
          <td>
            <i class="fas fa-user"></i> - <?php echo $f['name']?> 
            <br>
            <i class="fas fa-phone"></i> - <?php echo $f['phone']?>
            <br>
            <i class="fas fa-envelope"></i> - <?php echo $f['email']?>
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <center>
              ...
              <a href="contact.php?delete&id=<?=$f['id']?>" class="btn btn-danger">YES</a>
            </center>
            <p></p>
          </div>
        </div>
    ...
    <?php
      if(isset($_GET['delete'])){
        $id = $_GET['id'];
        $ins = $pdo->prepare("DELETE FROM contact WHERE id = '$id'");
        $ins->execute();
        if($ins){
           echo'<script>swal("Deleted", "Message deleted Successfully !", "success");</script>';
