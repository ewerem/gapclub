  ...
  <body>
    ...
    <div class="container">
      ...
      <h5>
        ...
        (<?php
          $get = $pdo->prepare("SELECT * FROM member");
          $get->execute();
          echo $get->rowCount();
        ?>) - Active Members
      </h5>
      ...
      <table id="table" class="table table-striped table-bordered">
        <tr>
          ...
          <td style="color: black;">
            ...
            <a data-toggle="modal" data-target="#deletemodal<?=$f['id']?>" title="Delete" class="btn btn-sm btn-danger" style="float: right;margin-right:10px;padding: 5px;color: white;margin-top: -50px;">
          ...
          <div class="modal-content">
            <div class="modal-body">
              ...
              <center>
                ...
                <a href="amember.php?deactivate&id=<?=$f['id']?>" class="btn btn-success">YES</a>
              </center>
              <p></p>
            </div>
          </div>
    ...
    <?php
      if(isset($_GET['deactivate'])){
        ...
        $get = $pdo->prepare("SELECT * FROM member WHERE id = '$id'");
        $get->execute();
        if($get->rowCount() > 0){
          while($f = $get->fetch()){
            $cphoto =  '../'.$f['cphoto'];
        ...
        unlink($cphoto);
        unlink($photo);
        $del = $pdo->prepare("DELETE FROM member WHERE id = '$id'");
        $del->execute();
        if($del){
           echo'<script>swal("Deleted", "Member deleted Successfully !", "success");</script>';
           echo'<meta http-equiv="refresh" content="2; url=amember.php">';
        }
